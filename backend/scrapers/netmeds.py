...
def scrape_netmeds(query):
    try:
        ...
        match = df[(df['medicine_name'].str.lower().str.contains(search_key, na=False)) & (df['platform'] == 'Netmeds')]
        ...
        return [
            {"name": match.iloc[0]['medicine_name'], "price": live_price, "platform": "Netmeds"}
        ]
    except Exception as e:
        print(f"Netmeds scrape failed: {e}")
        return []
