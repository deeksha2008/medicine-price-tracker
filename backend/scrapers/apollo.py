...
def scrape_apollo(query):
    try:
        ...
        match = df[(df['medicine_name'].str.lower().str.contains(search_key, na=False)) & (df['platform'] == 'Apollo Pharmacy')]
        ...
        return [
            {"name": match.iloc[0]['medicine_name'], "price": live_price, "platform": "Apollo Pharmacy"}
        ]
    except Exception as e:
        print(f"Apollo Pharmacy scrape failed: {e}")
        return []
